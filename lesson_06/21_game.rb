...
def calculate_two_card_ace(card_total)
  ace_value = [11, 1]
  ace_count = card_total.select(&:nil?)
...
def calculate_single_card_ace(card_total)
...
loop do
  ...
  loop do
    ...
    if draw_count == 4
      dealer_total, player_total =
        convert_face_to_digit([dealer_cards, player_cards])
      ...
      if card_total.flatten.include?(nil)
        calculate_single_card_ace(player_total)
      end
  ...
  loop do
    ...
    card_total = convert_face_to_digit([drawed_card])
    calculate_single_card_ace(dealer_total)
